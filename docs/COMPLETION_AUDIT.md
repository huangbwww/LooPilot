...
| Public internet access | `server/tunnel.mjs` downloads/caches private `cloudflared`, starts a trycloudflare tunnel in HTTP/2 mode, and prints the public URL without embedding the access token. Phone sign-in uses the pairing code or an explicitly pasted token. `README.md` documents `npm run dev:public`. `test/tunnel.test.mjs` verifies public tunnel output does not embed tokens. `test/server.integration.test.mjs` runs `server/index.mjs --public` with `LOOPILOT_FAKE_TUNNEL_URL` so no network or cloudflared process starts, verifies that the public URL line has no token, public `/api/health` reports `publicMode: true`, and pairing still returns the configured token. | Implemented with token-leak prevention and no-network public-mode startup-path coverage. Quick real tunnel was verified once before the reported blue screen; not re-run afterward. |
...
- `npm run lint`
- `npm test` currently reports 28 passing tests.
- `npm run build`
- `npm run doctor`
- `npm audit --omit=dev`
