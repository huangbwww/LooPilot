...
| --- | --- | --- |
| Existing Codex conversations | `server/codexStore.mjs` reads `~/.codex/session_index.jsonl` and rollout JSONL files under `~/.codex/sessions`. `npm run doctor` found rollout files in the local Codex home. `npm run accept:safe` started the local server in queue mode and authenticated `/api/sessions` returned local Codex sessions. `npm run accept:browser` rendered the production PWA against local Codex sessions. `test/codexStore.test.mjs` validates fixture parsing. | Implemented and tested with fixture plus local doctor, safe-mode runtime, and browser-render evidence. |
...
| Public internet access | `server/tunnel.mjs` downloads/caches private `cloudflared`, starts a trycloudflare tunnel in HTTP/2 mode, and prints the public URL without embedding the access token. Phone sign-in uses the pairing code or an explicitly pasted token. `README.md` documents `npm run dev:public` and `npm run accept:public`. `test/tunnel.test.mjs` verifies public tunnel output does not embed tokens. `test/server.integration.test.mjs` runs `server/index.mjs --public` with `LOOPILOT_FAKE_TUNNEL_URL` so no network or cloudflared process starts, verifies that the public URL line has no token, public `/api/health` reports `publicMode: true`, and pairing still returns the configured token. `scripts/accept-public.mjs` provides a one-command real tunnel acceptance check that waits for a real, non-API trycloudflare URL, verifies local readiness, then verifies `/api/health`, pairing, authenticated sessions, and `/live` WebSocket through the printed public URL. The script uses random credentials, redacts captured startup logs on failure, validates cached `cloudflared` by running `--version`, and `server/index.mjs` keeps the tunnel handle so shutdown can kill it. `test/acceptScripts.test.mjs` runs that script with a fake tunnel URL and fixture Codex home to verify the command completes, does not print `token=`, and redacts credentials on tunnel URL validation and assertion failures. `test/server.integration.test.mjs` also verifies shutdown exits with an active `/live` WebSocket client. | Implemented with token-leak prevention, no-network public-mode startup-path coverage, and real public URL acceptance evidence. On 2026-05-13 after the reported blue screen, `npm run accept:public` completed successfully, printed `OK public acceptance`, produced a token-free trycloudflare URL, verified API and WebSocket through that public URL with `verifiedViaPublicUrl: true`, then exited. |
| One-command startup | `npm run dev`, `npm run dev:safe`, `npm run dev:public`, `npm run accept:safe`, `npm run accept:browser`, `npm run accept:public`, and `npm run accept:bridge` exist. `README.md` documents the runtime commands and `docs/ACCEPTANCE_RUNBOOK.md` documents the acceptance commands. Startup prints an authorized URL and a 6-digit pairing code. `server/doctor.mjs` reports token and pairing-code readiness without exposing the pairing code. `server/state.mjs` falls back from project `.loopilot` to user state and temp state when directories are not writable. `server/index.mjs` puts Vite middleware cache under the LooPilot state directory. | Implemented and covered by state plus local server integration tests. Manual safe-mode startup reached the authorized URL after these fixes. On 2026-05-13 after the reported blue screen, `npm run accept:safe` completed with `OK safe acceptance`, `npm run accept:browser` rendered the production PWA in a 390x844 headless Chromium viewport and wrote `.tmp/accept-browser-mobile.png` plus `.tmp/accept-browser-mobile.html`, `npm run accept:public` completed with `verifiedViaPublicUrl: true`, and `npm run accept:bridge` completed with `bridgeStatus: "sent"`. |
| PWA | `public/manifest.webmanifest`, `public/sw.js`, and `public/icon.svg` exist. Service worker cache is versioned as `loopilot-v3` so installed PWAs receive the current pairing/security updates. Service worker avoids API/WebSocket fallback. `test/mobilePwa.acceptance.test.mjs` verifies install metadata, offline shell assets, cache version, service worker registration, and API/WebSocket bypass. `npm run accept:browser` renders the built PWA in Chromium and captures a mobile screenshot. | Implemented and covered by static PWA acceptance tests plus browser-render acceptance. |
| Mobile interaction model | `src/main.jsx` and `src/styles.css` implement mobile-first layout, left drawer, session list, timeline, status metrics, model/reasoning selectors, composer, auth gate, notification button, and queue-mode indicator. `test/mobilePwa.acceptance.test.mjs` verifies drawer state, scrim dismissal, safe-area rules, `100svh`, session switching, model/reasoning controls, textarea, disabled send state, and send payload wiring. `npm run accept:browser` verified a production mobile render containing the app shell, session rows, composer, model selector, reasoning selector, queue badge, and composer placeholder, and produced a 390x844 screenshot. | Implemented and covered by static mobile acceptance tests plus browser screenshot acceptance. |
...
- `npm run lint`
- `npm test` currently reports 37 passing tests.
- `npm run build`
- `npm run doctor`
- `npm audit --omit=dev`
...
Run this after `npm run build` to verify the production PWA in a mobile Chromium viewport without public tunnels or real Codex bridge sends:
...
Runtime acceptance completed on 2026-05-13:
...
- Authenticated `GET /api/sessions` returned local Codex sessions.
- Authenticated `/live` WebSocket returned a snapshot containing local sessions.
- `npm run accept:browser`, which starts the production server in queue mode, renders the PWA in a 390x844 headless Chromium viewport, verifies the rendered app shell/session/composer/model/reasoning/queue surfaces, and writes `.tmp/accept-browser-mobile.png` plus `.tmp/accept-browser-mobile.html`.
- `npm run accept:public`, which started the public tunnel flow in queue mode, printed `OK public acceptance`, produced a token-free trycloudflare URL, verified `/api/health`, pairing, authenticated sessions, and `/live` WebSocket through the public URL with `verifiedViaPublicUrl: true`, then exited. Follow-up process checks found no listener remaining on the acceptance port and no `cloudflared` process listed.
- `npm run accept:bridge`, which started app-server bridge mode with CLI fallback disabled, targeted explicit session `019e2050-3840-7893-abc1-4ef5c04c6f47`, and reported `OK bridge acceptance` with `bridgeStatus: "sent"`.

## Known Unverified Items

No known unverified items remain after the 2026-05-13 safe, browser, public tunnel, and real bridge acceptance runs.
...
Use this for regression checks after future changes. Follow `docs/ACCEPTANCE_RUNBOOK.md` when rerunning final acceptance after another system restart.
